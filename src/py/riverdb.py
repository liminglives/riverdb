...
class RiverDB(object):
    ...
    def np_dtype_convert2(dtype):
        if dtype in RiverDB.NPTypeDict:
            return RiverDB.NPTypeDict[dtype]
        return None

    @staticmethod
    def np_dtype_convert(dtype):
        if dtype == np.int16:
            return RiverDB.INT16
        elif dtype == np.int32:
            return RiverDB.INT32
        elif dtype == np.int64:
            return RiverDB.INT64
        elif dtype == np.uint16:
            return RiverDB.UINT16
        elif dtype == np.uint32:
            return RiverDB.UINT32
        elif dtype == np.uint64:
            return RiverDB.UINT64
        elif dtype == np.float32:
            return RiverDB.FLOAT
        elif dtype == np.float64:
            return RiverDB.DOUBLE
        elif dtype == np.object_:
            return RiverDB.STRING
        else:
            return None
    ...
    def dump_from_dataframe(file_name, df):
        ...
        for col in col_dtype.iteritems():
            name = col[0]
            t = RiverDB.np_dtype_convert(col[1])
            if t == None:
    ...
    def to_dataframe(file_name):
        b = riverdb_py.reader(file_name)
        row_num = b.row_num();
        dtypes = b.dtype()
        col_list = []
        for i in range(b.col_num()):
            col_list.append(np.empty(row_num, dtype=RiverDB.DataTypeDict[dtypes[i]]["np_dtype"]))
        line_no = 0
        for row in b:
            if line_no >= row_num:
                print "error"
...
class DictReader:
    def __init__(self, f):
        self._reader = riverdb_py.reader(f)
        if not self._reader:
...
class DictWriter:
    def __init__(self, f):
        self.__f = f
        self._writer = riverdb_py.writer(f)
        self.header = None
